...
class Boss(Enemy):
    def __init__(self, all_sprites, x, y, width, height):
        ...
        self.special_attack_cooldown = (
            2000  # Cooldown in milliseconds for special attack
        )
        ...
        if current_time - self.special_attack_timer >= self.special_attack_cooldown:
            ...
            for angle in range(0, 360, 45):
                bullet = Bullet_Player(
                    self.all_sprites, self.rect.centerx, self.rect.centery, angle
                )
                self.bullets.add(bullet)
            self.special_attack_timer = current_time
            self.last_special_attack_time = (
                current_time  # Update last special attack time
            )
    ...
    def update(self, camera_x=0):
        ...
        current_time = pygame.time.get_ticks()
        if (
            current_time - self.last_special_attack_time >= self.sword_delay
            and self.last_special_attack_time > 0
        ):
            self.sword_attack()
